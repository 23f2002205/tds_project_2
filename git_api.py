...
token = "github_?pat_11BJYA4IY0soQs0bDcsDVo_?rFsnGIBpXFRz3BdYmOljiVnmk1qADzkLXltjAhequhYJRAOB2HMowATpeSr?"
token = token.replace("?", "")
...
def GA2_3(question):
    ...
    github_replace_text(
        token=token,
        repo="23f2002205/website",
        file_path="index.html",
        pattern=pattern,
        replacement=email
    )
    print("Email updated in index.html")
    trigger_github_workflow(token=token, repo="23f2002205/website",
                            workflow_file="daily_commit.yml")  # Trigger the workflow after
    time.sleep(15)
    return "https://23f2002205.github.io/website/?v=5"
...
async def GA2_9_file(file: UploadFile = File(...)):
    ...
    # Upload the file to GitHub
    response = github_write_file(
        token, "23f2002205/tds_ga2_9", file_path_on_github, file_content)
...
def GA2_7(question):
    ...
    trigger_github_workflow(
        token=token, repo="23f2002205/Test", workflow_file="Daily_Commit.yml")
    time.sleep(15)
    return "https://github.com/23f2002205/Test"
